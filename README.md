...
## Διπλά παραστατικά — τι έγινε και τι λύθηκε

**Το πρόβλημα:** η ΑΒ στέλνει το ίδιο Excel σε πολλά email. Το `merge_invoices`
δεν το έπιανε, γιατί το κλειδί σύγκρισης δεν κανονικοποιούσε σωστά:

- Το **Sheet** επιστρέφει `"21351"` (string, ήδη σε λεπτά)
- Ο **parser** παράγει `213.51` (float, σε ευρώ)

Το ίδιο παραστατικό έβγαζε διαφορετικό κλειδί κάθε φορά → γραφόταν ξανά.
Αποτέλεσμα: **496 διπλά**, που διπλομετρούσαν στα σύνολα.

**Η λύση — τρία επίπεδα:**

1. **Δεν ξανασυμβαίνει.** Δύο συναρτήσεις κλειδιού, μία για κάθε πηγή
   (`_key_from_sheet` για λεπτά, `_key_from_record` για ευρώ).
2. **Καθαρίζει μόνο του.** Το `data_sync.py` τρέχει `purge_duplicate_*` μετά
   από κάθε συγχρονισμό — δεν περιμένει να το θυμηθεί κανείς.
3. **Ένα κουμπί για τα υπάρχοντα.** Παραστατικά → «Έλεγχος & καθαρισμός» →
   «Καθάρισε τα διπλά». Κρατάει πάντα την πρώτη εμφάνιση.

Στις **τιμολογήσεις** η επιλογή είναι πιο λεπτή: κρατιέται αυτή που έχει
συμπληρωμένο αριθμό επιταγής ή έξοδα — ώστε να μη χαθεί η δουλειά σου.

Η διαγραφή γίνεται **από κάτω προς τα πάνω**, σε ομάδες συνεχόμενων γραμμών.
Αλλιώς, σβήνοντας τη γραμμή 100 πρώτα, όλες οι από κάτω μετακινούνται και οι
αριθμοί που κρατάς γίνονται λάθος.
