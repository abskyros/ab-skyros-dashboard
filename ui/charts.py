...
def year_over_year(
    ...
    *,
    ...
    fig.add_trace(go.Scatter(
        ...
        name=label_now,
        mode="lines",
        line=dict(color=COLOR["brand"], width=2.6, shape="spline", smoothing=0.5),
        fill="tozeroy",
        fillcolor="rgba(0,114,206,.07)",
...
def bars(labels: list[str], values: list[float], *, height: int = 260, color: str | None = None) -> None:
    ...
    fig = go.Figure(go.Bar(
        x=labels, y=values,
        marker=dict(color=color or COLOR["brand"], line=dict(width=0)),
...
def paired_bars(
    ...
    *,
    ...
    fig.add_trace(go.Bar(
        ...
        marker=dict(color=COLOR["brand"], line=dict(width=0)),
...
def daily_week(dates: list, values: list[float], *, height: int = 220) -> None:
    ...
    colors = [
        COLOR["ink"] if d.weekday() == 5 else COLOR["brand"]
        for d in dates
    ]
