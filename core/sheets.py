...
def purge_duplicate_invoices() -> tuple[int, int]:
    """
    Σβήνει τα διπλά παραστατικά. Κρατάει την ΠΡΩΤΗ εμφάνιση κάθε εγγραφής.

    → (πόσα σβήστηκαν, πόσα έμειναν)

    Γιατί υπάρχει: η ΑΒ στέλνει το ίδιο Excel σε πολλά email. Πριν φτιαχτεί το
    κλειδί, το ίδιο παραστατικό γραφόταν 2-3 φορές, και τα σύνολα βγαίναν λάθος.

    ΓΙΑΤΙ ΣΒΗΝΟΥΜΕ ΑΠΟ ΚΑΤΩ ΠΡΟΣ ΤΑ ΠΑΝΩ: αν σβήσεις τη γραμμή 100 πρώτα, όλες
    οι από κάτω μετακινούνται μία θέση πάνω και οι αριθμοί που κρατάς είναι πια
    ...
    for i, r in enumerate(vals[1:], start=2):
        ...
        key = _key_from_sheet(r)
        if key in seen:
            doomed.append(i)     # το ξαναείδαμε → φεύγει
        else:
            seen.add(key)        # πρώτη φορά → μένει

    if not doomed:
        return 0, len(seen)

    # Από κάτω προς τα πάνω, σε ομάδες συνεχόμενων γραμμών (1 κλήση αντί για 500).
