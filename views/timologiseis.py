...
from core.sheets import check_quality, delete_row, purge_duplicate_timologiseis
...
def _tools() -> None:
    ...
    with st.expander("Έλεγχος δεδομένων"):
        ...
        if dups:
            ...
            c.note(
                f"<b>{len(dups)} ημερομηνίες επιταγής</b> υπάρχουν πάνω από μία φορά "
                f"— <b>{extra} περιττές γραμμές</b>.",
                "warn",
            )
            st.caption(
                "Κρατιέται αυτή που έχει συμπληρωμένο αριθμό επιταγής ή έξοδα, "
                "ώστε να μη χαθεί η δουλειά σου."
            )

            if st.button(f"Καθάρισε τα {extra} διπλά", key="timol_purge",
                         width="stretch", type="primary"):
                with st.spinner("Διαγραφή…"):
                    killed, kept = purge_duplicate_timologiseis()

                if killed:
                    c.note(f"Σβήστηκαν {killed} γραμμές. Έμειναν {kept} επιταγές.", "ok")
                    st.session_state["timol_checked"] = False
                    st.rerun()
                else:
            ...
            with st.expander("Δες τα αναλυτικά", expanded=False):
                for d in dups[:20]:
                    ...
                    st.markdown(f"**{d['date']}** — {len(d['entries'])} φορές ({lines})")
